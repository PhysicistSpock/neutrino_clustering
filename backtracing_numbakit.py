...
def EOMs(s, y):
    ...
    if s in s_steps:
        z = np.array(CC.ZEDS[s_steps==s][0], dtype=np.float64)
    else:
        # z = s_to_z(s)  # interpolation function defined below

        # Using numpy interpolation to work with numba.
        z = np.array(np.interp(s, s_steps, CC.ZEDS), dtype=np.float64)
    ...
    dyds_xi = u_i
    dyds_ui = signs * 1./(1.+z)**2. * gradient 

    dyds = CC.TIME_FLOW * np.concatenate((dyds_xi, dyds_ui))

    # if z==0.:
    #     print(dyds.shape, type(dyds), dyds)
